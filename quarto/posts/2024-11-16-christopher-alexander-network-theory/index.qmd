...
[A figure from NotSoF which shows design variables (small black circles, so-called _nodes_ in the network) and their interactions (lines). The nodes are grouped into clusters (represented as the larger circles).]{.column-margin}
![Diagrams](diagrams.jpg){width="80%"}
