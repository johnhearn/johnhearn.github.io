...
categories: [software-testing, games, software-engineering]
---
...
[I later realised that a better acceptance test would also include something like print the name of the winner. The test above doesn't involve any interface to a user at all making the game model fairly useless.]{.column-margin}
...
[Note: a Uno game actually ends when a player has scored 500 or more points. What we're dealing with here is a _single round_ in a game. That's something I realised later and was able to resolve by renaming it to `Round`.]{.column-margin}
...
[Note to self: should have committed to version control here.]{.column-margin}
...
[Having discovered that `Pack` needed a `takeCard()` method, I didn't consider whether that method should have been tested with it's own test cases. This is something that should be done whenever generating new methods beyond those mentioned in the latest test case.]{.column-margin}
...
[I realised much later that I should have named the class `NumberedCard` at this point. Calling it `Card` is too generic and makes refactoring a common base class more tricky later on. TDD is hard. Good OO is hard. They are skills that need to be learned through practice.]{.column-margin}
...
[Just noticed how ugly this is :/]{.column-margin}
```java
...
Continued [here](../2017-05-23-notes-on-art-of-tdd-uno-part-2/).