...
Grover's algorithm is actually useful[Useful, at least, if we had more real qubits available for processing. That is to say reaching quantum supremacy. All of these examples only make sense when scaled to hundreds of qubits. We currently have just a few dozen at most, not counting for error correction. ]{.column-margin}. It can quickly find configurations of bits matching some predefined criteria. Let's take an absurdly simple example and find the bit sequence matching a given number.
...
![grovers-circuit](grovers-circuit.png)
