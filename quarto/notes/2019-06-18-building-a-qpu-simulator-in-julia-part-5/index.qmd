...
[Grover's] search is the next algorithm to implement in with Julia. It's not that much more complex that the Deutch-Jozsa algorithm from the [last post](../../notes/2019-06-17-building-a-qpu-simulator-in-julia-part-4/) but it is an example of an algorithm that is potentially very useful for quantum circuits. It enables us to search for states satisfying a given set of conditions[^queens] much more quickly, $\mathcal{O}(\sqrt{N})$, than its classical counterpart, $\mathcal{O}(N)$.
...
![Grover's search algorithm circuit](grovers-circuit.png)
...
[^queens]: A toy example of which might be the [N-Queens problem](https://www.google.com/search?q=n-queens+problem+quantum+computing) for very large boards.
