...
Then create a basic Maven[^maven] project and add the minimum dependencies:

[It's important that the Slf4j implementation you choose matches the version specified by `sparkjava`. I had trouble when it didn't.]{.column-margin}
...
[^currently]: I'm sure that someone will come up with the [appropriate workarounds](https://jira.spring.io/browse/SPR-16991) to overcome this problem, a problem that doesn't even need to exist.
[^maven]: I chose to use Maven because I know it better. I'd like to do it also with Gradle.
[^link]: Compare these results with a Go microservice from a [previous post](../2017-11-01-notes-on-go-go-kit-for-java-programmer/).
