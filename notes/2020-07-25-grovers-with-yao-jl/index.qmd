---
categories: [quantum-computing, programming-languages]
...
In the tutorial they use the phased version of the diffusion operator[This diagram is generated directly from the code with YaoDraw :)]{.column-margin}:

![grovers-circuit](grovers-circuit-phase.png){width="400px"}
...
The variable `iter` is the number of iterations we want to apply. Getting the right value for this is tricky[The tutorial has much more sophisticated way of doing this.]{.column-margin} so we just use a hand picked value for now. It should be less than $\sqrt{2^n}$. For clarity we wrap the whole circuit in a function and pass in the oracle:
...
![grovers-search](grovers-search-anim.gif)
